...
```jsonc
{
  ...
  "runtime": {
    ...
    "fillRankChange": {
      ...
      // 알림이 나가면 Discord 본문에 minStakeDeltaSol 이상 변화라고 이유가 표시됨.
      "minStakeDeltaSol": 1000,

      // Bid / Bid @5/0 변화가 이보다 작으면 무시.
      // 알림이 나가면 Discord 본문에 minBidDeltaPmpe 이상 변화라고 이유가 표시됨.
      "minBidDeltaPmpe": 0.0005,

      // Fill 비율 변화가 이보다 작으면 무시.
      // 0.01 = 1%p.
      // 알림이 나가면 Discord 본문에 minFillPctDelta 이상 변화라고 이유가 표시됨.
